...
import 'package:flutter_rtc/src/call_screen/call_content/picture_in_picture/stream_picture_in_picture_ui_kit_view.dart';
...
class _StreamCallContentState extends State<StreamCallContent>
  ...
  void didUpdateWidget(covariant StreamCallContent oldWidget) {
    ...
    if (widget.callState.localParticipant?.isScreenShareEnabled !=
      ...
      if (widget.pictureInPictureConfiguration.disablePictureInPictureWhenScreenSharing) {
  ...
  Widget build(BuildContext context) {
    ...
    final pipEnabled =
        widget.pictureInPictureConfiguration.enablePictureInPicture &&
        (!widget.pictureInPictureConfiguration.disablePictureInPictureWhenScreenSharing ||
            !(callState.localParticipant?.isScreenShareEnabled ?? false));

    if (pipEnabled && _isPictureInPictureModeOn && Platform.isAndroid) {
      return widget
    ...
    if (callState.status.isConnected ||
      ...
      bodyWidget = Stack(
        children: [
          ...
          widget.callParticipantsBuilder?.call(context, call, callState) ??
              StreamCallParticipants(
                call: call,
                participants: callState.callParticipants,
      ...
      bodyWidget = Center(child: Text(statusText, style: theme.textTheme.title3));
    }
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          GestureDetector(onDoubleTap: _toggleStatsVisibility, child: bodyWidget),
          Visibility(
